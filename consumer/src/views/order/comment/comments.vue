<template>
  <div id="comments" style="background-color: #f7f7f7">
    <van-nav-bar title="订单评论" left-arrow  @click-left="$router.go(-1)"/>
    <div class="comment-container">
      <div class="cm-item">
          <div class="cm-nav">
            <img :src="orderImage" class="cm-img">
            <div class="cm-grade">
                <span class="item-flower praise" :class="[commentsForm.grade === 'good' && 'active']">
                  <input type="radio" v-model="commentsForm.grade" value="good">
                  <i></i>
                  <span>好评</span>
                </span>
              <span class="item-flower average" :class="[commentsForm.grade === 'neutral' && 'active']">
                  <input type="radio" v-model="commentsForm.grade" value="neutral">
                  <i></i>
                  <span>中评</span>
                </span>
              <span class="item-flower bad-review" :class="[commentsForm.grade === 'bad' && 'active']">
                  <input type="radio" v-model="commentsForm.grade" value="bad">
                  <i></i>
                  <span>差评</span>
                </span>
            </div>
...
<!--        图片上传-->
          ...
          <div class="shop-grade">
            ...
            <div class="rating-star">
              <div class="logistics-star">
                ...
                <div class="star">
                  <van-rate v-model="commentsForm.business_score" color="#f42424"/>
                </div>
                <div class="grade-text">
                  <span>{{ countGradeText(commentsForm.business_score) }}</span>
                </div>
              </div>
              <div class="logistics-star">
                <div class="text">
    ...
    <van-button type="danger" size="large" @click="submitForm" class="cm-btn">发表评论</van-button>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    mounted() {
      API_Order.orderDetail(this.order_sn).then(response => {
        this.order=response
        //订单图片为第一张
        this.orderImage=this.order.order_images.split(',')[0];
      })
    },
    methods: {
      countGradeText(val) {
      ...
      submitForm() {
        ...
        if (params.grade !== 'goods' && !params.content) {
          err_flg = true
        ...
        for(let j = 0,len=this.fileList.length; j < len; j++) {
          if(j===len-1){
            params.images+=this.fileList[j].url
          }else{
            params.images+=this.fileList[j].url+','
          }
        }
        params.order_sn=this.order_sn
        API_Order.commentOrder(params).then(() => {
...
<style>
  ...
  .el-upload--picture-card {
    ...
    width: 80px;
    line-height: 80px;
    background-image: url("../../../assets/img/xiangji.jpg") ;
