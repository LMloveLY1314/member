<template>
  <div id="goods-comments" class="goods-comments">
      ...
      <div v-else class="some-comments">
        <template v-for="(comment, index) in comments">
          <div v-if="index < 3" :key="index" class="com-item">
            <div class="com-user">
              <span class="user">{{ comment.consumer_name }}</span>
              <span class="credit">{{ comment.grade | filterGrade }}</span>
              <span class="date">{{ comment.comment_time | unixToDate('yyyy-MM-dd') }}</span>
            </div>
            <div class="com-content">{{ comment.content }}</div>
            <div v-if="comment.images" class="com-gallery">
            <span v-for="(img, index) in comment.images.split(',')" :key="index" class="img">
    ...
    <van-popup v-model="showPopup" position="bottom" style="height:100%">
      ...
      <van-empty v-if="finished && !comments.length" class="no-comments">暂无评价</van-empty>
      <van-list
        v-else
        v-model="loading"
        :finished="finished"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      GET_OrderComments(){
        this.loading = true
        this.params.repair_id=this.repairId
